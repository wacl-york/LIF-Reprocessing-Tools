...
<<<<<<< HEAD
    ...
    for channel in channel_format:
        ...
        #EDIT: FROM BEFORE CHRISTMAS! 09/01/2026
            pad_width = (int(shift_value), 0)
...
<<<<<<< HEAD
    for channel in channels:
        ...
        data['cal_true_ppt'] = data['SO2_mr'].where(
        ...
        for cal in range(1,cal_num+1):
            ...
            cal_tmp_df = data[(data['cal_group'] == cal)].copy().dropna(
               subset=[cts_diff_v, cts_diff_refnorm_v, 'SO2_mr', 'cal_true_ppt'
                       , 'Cal_SO2_MFC_set', 'lsr_pwr_mW']
            )
            
            avg_lsr_pwr = cal_tmp_df['lsr_pwr_mW'].mean()
            
            cal_tmp_df['point_filter'] = 0
            cal_tmp_df['cal_flow_diff'] = cal_tmp_df['Cal_SO2_MFC_set'].diff().abs()
